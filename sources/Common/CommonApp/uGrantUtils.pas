...
Type
  /// <summary>
  /// TGrant - клас дл€ работы с правами пользовател€
  /// </summary>
  TGrant = class
  private
  ...
  public
    ...
    /// <summary>
    /// GrantTemplateCreate - —оздание шаблона меню.
    /// —охран€ет наименовани€ пунктов actions в таблицу tMenu.
    /// </summary>
     procedure GrantTemplateCreate(AComp: TComponent; AParentMenu: string = '');

    /// <summary>
    /// UserGrantLoad - загрузка прав пользовател€ в пам€ть
    /// </summary>
     procedure UserGrantLoad();

    /// <summary>
    /// SetGrant - применени прав пользовател€ к меню системы
    /// </summary>
...
begin
  ...
  Begin
    try
      ...
      on E: Exception do
      begin
        log(Format('ќшибка  [%s]', [E.Message]), etException);
        log(Format('ќшибка  [%s]', [FUserGrant.Values[AComp.ClassName + '.' + vartostr(AAction[Index].Name)]]), etException);
      end;
    end;
  end;
end;
