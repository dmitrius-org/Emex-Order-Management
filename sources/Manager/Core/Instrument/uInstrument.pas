...
type
  /// <summary>
  /// TInstrumentData - объект для хранения дополнительных данных по Instrument в
  ///  дереве меню
  /// </summary>
  /// <seealso>TInstrumentData</seealso>
  PInstrumentData = ^TInstrumentData;
  TInstrumentData= record
    /// <summary> InstrumentID - ИД инструмента </summary>
    ID    : Integer;
    /// <summary> InstrumentType - Тип инструмента </summary>
    IType  : Integer;
    /// <summary> Form - наименование класса формы </summary>
    Form            : string;
    /// <summary> PID - Ид родительского элемента. для tModel это TInsrument.InstrumentID </summary>
    PID : Integer;
    SID : Integer;
    Flag: Integer;
  //  ModelID: Integer;
...
//    function FindNodeByID(AID: Integer): TUniTreeNode;
    ...
    /// <summary>
    ///  SetNodeIco - Установка иконки для ветки в зависимости от типа
    ///</summary>
    procedure SetNodeIco( Nd : TUniTreeNode);
    /// <summary>
    ///  SetNodeEditFormName - Установка формы редактирования для ветки в зависимости от типа ветки
    ///</summary>
...
//     begin
//       MessageDlg('У действия уже настроен переход в состояние!', mtError, [mbOK]);
//       exit
//     end;
//  end;
...
begin
  ...
  begin
    ...
    begin
       // если идентификатор изменился, то нужно обновить данные в форме
