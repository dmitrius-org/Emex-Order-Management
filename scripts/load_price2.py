...
for file in file_list: 
    if fnmatch.fnmatch(file, sys.argv[1]): #  EMIR FAST 
        ...
        if 1==1:    
            ...
            logger.info(f'Загрузили файл {file} в объект DataFrame. Время выполнения  {u.t(toc - tic)}')
            
            sql.load_prices(data=df, batchsize=100000)
            # df = df[0:0]  
            df.iloc[0:0]           
            toc = time.perf_counter()
            logger.info(f"Завершение обработки файла {file}. Вычисление заняло {u.t(toc - tic)}")
            processed = True
